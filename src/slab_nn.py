...
class SlabSystem:
    ...
    def select(self, system_type):
        match system_type:
            ...
            case "DEV-IM":
                self.set_specs(
                    8, 8, 1, 0, 16, 26.0
                )  # Dev IMAS system, with just one supermodule
            case "DEV-EB":
                self.set_specs(
                    16, 8, 2, 0, 16, 26.0
                )  # Dev EBRAINS system, with just one supermodule
            case _:
    ...
    def set_specs(self, slabs, SiPM, slabs_per_si, rings, mmodules, width, groups=0):
        ...
        self.SLABS_PER_SIPM = (
            slabs_per_si  # Number of slabs per silicon (silicon cols).
        )
    ...
    def set_grouping(self, groups):
        total_slabs = (
            self.N_SLABS * self.N_MODULES
        )  # Total number of slabs in system (for a supermodule, it is N_SLABS*16)
        sortedlist = np.arange(
            total_slabs
        )  # Create the array with the number of slabs to be filled with mapping indexes

        match groups:
            case (
                0
            ):  # No grouping, each slab has its own category, which is unique for all slabs in the system
                # Slab category is actually slab index
                mapping = sortedlist
            case (
                1
            ):  # SiPM grouping. All slabs sharing SiPM are grouped in the same category.
            ...
            case (
                2
            ):  # MM grouping. Each minimodule has its own category, all the slabs in the
                # same MM are grouped together.
                mapping = np.floor_divide(sortedlist, self.N_SLABS)
            case (
                3
            ):  # Position grouping. Each slab on the same position is in the same category.
...
class SlabNN:
    ...
    def build_combined(
        self, XY_model_file, DOI_model_file, print_model=True, name="Combined"
    ):
        print("SlabNN: Building combined model...")
        print(f"XY model: {XY_model_file}")
        print(f"DOI model: {DOI_model_file}")
        ...
        concatenated_output = keras.layers.Concatenate()(
            [model_XY.layers[-1].output, model_DOI.layers[-1].output]
        )

        model_name = name

        self.model = keras.Model(
            inputs=[(model_XY.inputs, model_DOI.inputs)],
        ...
        if print_model == True:
            ...
            keras.utils.plot_model(
                self.model, to_file=self.model.name + "-model.png", show_shapes=True
            )
    ...
    def train(
        ...
        if self.model is None:
            print(
                "SlabNN ERROR: Model not yet defined, please define model prior to training"
            )
            return

        # The patience parameter is the amount of epochs to check for improvement
        early_stop = keras.callbacks.EarlyStopping(
            monitor="val_loss", patience=100, min_delta=0.005
        )
    ...
    def predict(self, dataset, batch=16384, verbosity=0):
        ...
        if self.model is None:
            print(
                "SlabNN ERROR: Model not yet defined, please define model prior to predicting"
            )
            return
...
def _xy_local(
    slab_id: int, y_mm: np.float32, local_pos_dict: dict, loc_trans: bool
) -> tuple[float, float]:
    local_x, slab_y, _ = local_pos_dict[slab_id]
    # print(local_x, slab_y)
    ...
    local_pos = np.asarray(
        tuple(
            map(
                ...
                slab_ids,
                mm_y,
                [local_pos_dict] * len(slab_ids),
                [loc_trans] * len(slab_ids),
            )
        )
    )
